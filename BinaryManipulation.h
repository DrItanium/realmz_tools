...
namespace realmz {
    constexpr uint16_t make(uint8_t first, uint8_t second) noexcept {
        return ((static_cast<uint16_t>(first) << 8) & 0xFF00) | (static_cast<uint16_t>(second) & 0x00FF);
    }
    constexpr uint8_t upperHalf(uint16_t value) noexcept { return (value >> 8) & 0xFF; }
    constexpr uint8_t lowerHalf(uint16_t value) noexcept { return value & 0xFF; }
    ...
    constexpr uint64_t make(uint16_t a, uint16_t b, uint16_t c, uint16_t d, ConstructInt64) noexcept {
        ...
        return (static_cast<uint64_t>(a) & Masks[0]) |
               ((static_cast<uint64_t>(b) << 16) & Masks[1]) |
               ((static_cast<uint64_t>(c) << 32) & Masks[2]) |
               ((static_cast<uint64_t>(d) << 48) & Masks[3]);
    }
